...
def draw_text(ax, x, y, text, color=[0, 0, 0, 1], fontsize=14, ha="center", va="center"):
    ax.text(
        x, y, text,
...
def venn4_ax(ax, labels, names=['A', 'B', 'C', 'D'], **options):
    """
    plots a 4-set Venn diagram
    @type labels: dict[str, str]
    @type names: list[str]
    ...
    # legend
    draw_text(ax, 0.13, 0.18, names[0], colors[0], fontsize=fontsize, ha="right")
    draw_text(ax, 0.18, 0.83, names[1], colors[1], fontsize=fontsize, ha="right", va="bottom")
    draw_text(ax, 0.82, 0.83, names[2], colors[2], fontsize=fontsize, ha="left", va="bottom")
    draw_text(ax, 0.87, 0.18, names[3], colors[3], fontsize=fontsize, ha="left", va="top")
    if legend:
      leg = ax.legend(names, loc='center left', bbox_to_anchor=(1.0, 0.5), fancybox=True)
      leg.get_frame().set_alpha(0.5)
